...
function Project({ skills, title, date, location, body, img, links, right, caption }: ProjectProps) {
    ...
    return (
        <div className={`flex md:gap-12 justify-center flex-col items-start ` + (right ? 'md:flex-row-reverse' : 'md:flex-row')} style={{width: `min(${total}rem,90%)`}}>
            {img == ""
                ? <div style={{width: `min(${widths[0]}rem,100%)`}}></div>
            ...
            <div className={"h-full text-white "}
                ...
                <div className="w-full flex justify-between text-off-white tracking-widest fira-code-font text-content-sm md:mt-0 mt-2 text-nowrap flex-wrap">
                    <span>&#123; {date}</span>
                    <span className="text-right flex-grow text-wrap">{location} &#125;</span>
                </div>
                ...
                <div className="w-full text-off-white text-content-sm">
                    {window.innerWidth < 480
                        ? (<div className="flex mt-2 gap-2">
                            <div className={"w-[1px] bg-off-white " + (links.length == 0 ? "" : "translate-y-0.5")}></div>
                            <div className="py-[2px]">
                                <p className="">{skills}</p>
                                <div className="flex gap-2">
                                    {links.map(({ text, link }) => <a key={key++} className="link-red-2 font-semibold mt-1" href={link} target="_blank">{text}</a>)}
                                </div>
                            </div>
                        </div>)
                        : (<div className="flex justify-between items-center gap-4 mt-4">
                            {links.map(({ text, link }) => <a key={key++} className="link-red-2 font-semibold" href={link} target="_blank">{text}</a>)}
...
export default function Projects() {
    return (
        <div className="flex justify-center w-full flex-col items-center">
            ...
            <div className="h-16 sm:h-24 md:h-36"></div>
            <div className="flex gap-8 md:gap-16 w-[min(75rem,90%)] items-center">
